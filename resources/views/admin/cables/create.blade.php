...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.cable.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.cables.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="cable_size">{{ trans('cruds.cable.fields.cable_size') }}</label>
                <input class="form-control {{ $errors->has('cable_size') ? 'is-invalid' : '' }}" type="text" name="cable_size" id="cable_size" value="{{ old('cable_size', '') }}">
