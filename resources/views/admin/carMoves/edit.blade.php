...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.carMove.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.car-moves.update", [$carMove->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('car_no') ? 'is-invalid' : '' }}" name="car_no_id" id="car_no_id" required>
                    @foreach($car_nos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('car_no_id') ? old('car_no_id') : $carMove->car_no->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('last_driver') ? 'is-invalid' : '' }}" name="last_driver_id" id="last_driver_id">
                    @foreach($last_drivers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('last_driver_id') ? old('last_driver_id') : $carMove->last_driver->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('driver') ? 'is-invalid' : '' }}" name="driver_id" id="driver_id">
                    @foreach($drivers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('driver_id') ? old('driver_id') : $carMove->driver->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $carMove->date) }}">
                @if($errors->has('date'))
