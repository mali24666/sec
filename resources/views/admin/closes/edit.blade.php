...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.close.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.closes.update", [$close->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lices_no') ? 'is-invalid' : '' }}" name="lices_no_id" id="lices_no_id">
                    @foreach($lices_nos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lices_no_id') ? old('lices_no_id') : $close->lices_no->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
