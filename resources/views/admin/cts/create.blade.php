...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.ct.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.cts.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="ct_no">{{ trans('cruds.ct.fields.ct_no') }}</label>
                <input class="form-control {{ $errors->has('ct_no') ? 'is-invalid' : '' }}" type="text" name="ct_no" id="ct_no" value="{{ old('ct_no', '') }}">
                @if($errors->has('ct_no'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('point_1') ? 'is-invalid' : '' }}" name="point_1_id" id="point_1_id">
                    @foreach($point_1s as $id => $entry)
                        <option value="{{ $id }}" {{ old('point_1_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('point_2') ? 'is-invalid' : '' }}" name="point_2_id" id="point_2_id">
                    @foreach($point_2s as $id => $entry)
                        <option value="{{ $id }}" {{ old('point_2_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
