...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.diagram.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.diagrams.update", [$diagram->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('station') ? 'is-invalid' : '' }}" name="station_id" id="station_id">
                    @foreach($stations as $id => $entry)
                        <option value="{{ $id }}" {{ (old('station_id') ? old('station_id') : $diagram->station->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('feeders') ? 'is-invalid' : '' }}" name="feeders[]" id="feeders" multiple>
                    @foreach($feeders as $id => $feeder)
                        <option value="{{ $id }}" {{ (in_array($id, old('feeders', [])) || $diagram->feeders->contains($id)) ? 'selected' : '' }}>{{ $feeder }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cts') ? 'is-invalid' : '' }}" name="cts[]" id="cts" multiple>
                    @foreach($cts as $id => $ct)
                        <option value="{{ $id }}" {{ (in_array($id, old('cts', [])) || $diagram->cts->contains($id)) ? 'selected' : '' }}>{{ $ct }}</option>
                    @endforeach
                </select>
                @if($errors->has('cts'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('trans') ? 'is-invalid' : '' }}" name="trans[]" id="trans" multiple>
                    @foreach($trans as $id => $tran)
                        <option value="{{ $id }}" {{ (in_array($id, old('trans', [])) || $diagram->trans->contains($id)) ? 'selected' : '' }}>{{ $tran }}</option>
                    @endforeach
                </select>
                @if($errors->has('trans'))
