        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.minibller.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.minibllers.update", [$minibller->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('transformer') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="transformer_id" id="transformer_id">
                                @foreach($transformers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('transformer_id') ? old('transformer_id') : $minibller->transformer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('transformer'))
                                <span class="help-block" role="alert">{{ $errors->first('transformer') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.minibller.fields.transformer_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('cb') ? 'has-error' : '' }}">
                            <label for="cb_id">{{ trans('cruds.minibller.fields.cb') }}</label>
                            <select class="form-control select2" name="cb_id" id="cb_id">
                                @foreach($cbs as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('cb_id') ? old('cb_id') : $minibller->cb->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('cb'))
                                <span class="help-block" role="alert">{{ $errors->first('cb') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.minibller.fields.cb_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('minibller_number') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="minibller_number" id="minibller_number" value="{{ old('minibller_number', $minibller->minibller_number) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('longitude') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="longitude" id="longitude" value="{{ old('longitude', $minibller->longitude) }}">
                        ...
                        <div class="form-group {{ $errors->has('rating') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="rating" id="rating" value="{{ old('rating', $minibller->rating) }}">
                        ...
                        <div class="form-group {{ $errors->has('manufacturer_serial_no') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="manufacturer_serial_no" id="manufacturer_serial_no" value="{{ old('manufacturer_serial_no', $minibller->manufacturer_serial_no) }}">
                        ...
                        <div class="form-group {{ $errors->has('no_circuits_ici_kva_rating_manuf_ly') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="no_circuits_ici_kva_rating_manuf_ly" id="no_circuits_ici_kva_rating_manuf_ly" value="{{ old('no_circuits_ici_kva_rating_manuf_ly', $minibller->no_circuits_ici_kva_rating_manuf_ly) }}">
                        ...
                        <div class="form-group {{ $errors->has('no_of_used_circuits') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="no_of_used_circuits" id="no_of_used_circuits" value="{{ old('no_of_used_circuits', $minibller->no_of_used_circuits) }}">
                        ...
                        <div class="form-group {{ $errors->has('manufacturer') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="manufacturer" id="manufacturer" value="{{ old('manufacturer', $minibller->manufacturer) }}">
                        ...
                        <div class="form-group {{ $errors->has('cable_size') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="cable_size_id" id="cable_size_id">
                                @foreach($cable_sizes as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('cable_size_id') ? old('cable_size_id') : $minibller->cable_size->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('minibllar_notes') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="minibllar_notes[]" id="minibllar_notes" multiple>
                                @foreach($minibllar_notes as $id => $minibllar_note)
                                    <option value="{{ $id }}" {{ (in_array($id, old('minibllar_notes', [])) || $minibller->minibllar_notes->contains($id)) ? 'selected' : '' }}>{{ $minibllar_note }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('area_name') ? 'has-error' : '' }}">
                            <label for="area_name_id">{{ trans('cruds.minibller.fields.area_name') }}</label>
                            <select class="form-control select2" name="area_name_id" id="area_name_id">
                                @foreach($area_names as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('area_name_id') ? old('area_name_id') : $minibller->area_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('area_name'))
                                <span class="help-block" role="alert">{{ $errors->first('area_name') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.minibller.fields.area_name_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('other_note') ? 'has-error' : '' }}">
                            <label for="other_note">{{ trans('cruds.minibller.fields.other_note') }}</label>
                            <textarea class="form-control ckeditor" name="other_note" id="other_note">{!! old('other_note', $minibller->other_note) !!}</textarea>
                            @if($errors->has('other_note'))
                                <span class="help-block" role="alert">{{ $errors->first('other_note') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.minibller.fields.other_note_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('load') ? 'has-error' : '' }}">
                            <label for="load">{{ trans('cruds.minibller.fields.load') }}</label>
                            <input class="form-control" type="number" name="load" id="load" value="{{ old('load', $minibller->load) }}" step="0.01">
                            @if($errors->has('load'))
                        ...
                        <div class="form-group {{ $errors->has('load_b') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="load_b" id="load_b" value="{{ old('load_b', $minibller->load_b) }}">
                        ...
                        <div class="form-group {{ $errors->has('load_y') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="load_y" id="load_y" value="{{ old('load_y', $minibller->load_y) }}">
