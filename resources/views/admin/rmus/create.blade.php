...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.rmu.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.rmus.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="rmu_no">{{ trans('cruds.rmu.fields.rmu_no') }}</label>
                <input class="form-control {{ $errors->has('rmu_no') ? 'is-invalid' : '' }}" type="text" name="rmu_no" id="rmu_no" value="{{ old('rmu_no', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('rmu_feeder') ? 'is-invalid' : '' }}" name="rmu_feeder_id" id="rmu_feeder_id">
                    @foreach($rmu_feeders as $id => $entry)
                        <option value="{{ $id }}" {{ old('rmu_feeder_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
