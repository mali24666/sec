    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.rooms.update", [$room->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('station_no') ? 'is-invalid' : '' }}" type="text" name="station_no" id="station_no" value="{{ old('station_no', $room->station_no) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('neighborhood') ? 'is-invalid' : '' }}" type="text" name="neighborhood" id="neighborhood" value="{{ old('neighborhood', $room->neighborhood) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('coordinates') ? 'is-invalid' : '' }}" type="text" name="coordinates" id="coordinates" value="{{ old('coordinates', $room->coordinates) }}" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('work_lamp') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="work_lamp" id="work_lamp" value="1" {{ $room->work_lamp || old('work_lamp', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('fanwrok') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="fanwrok" id="fanwrok" value="1" {{ $room->fanwrok || old('fanwrok', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <label for="lamp_after">{{ trans('cruds.room.fields.lamp_after') }}</label>
                <div class="needsclick dropzone {{ $errors->has('lamp_after') ? 'is-invalid' : '' }}" id="lamp_after-dropzone">
                </div>
                @if($errors->has('lamp_after'))
                    <span class="text-danger">{{ $errors->first('lamp_after') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.room.fields.lamp_after_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="fan_after">{{ trans('cruds.room.fields.fan_after') }}</label>
                <div class="needsclick dropzone {{ $errors->has('fan_after') ? 'is-invalid' : '' }}" id="fan_after-dropzone">
                </div>
                @if($errors->has('fan_after'))
                    <span class="text-danger">{{ $errors->first('fan_after') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.room.fields.fan_after_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    var uploadedLampAfterMap = {}
Dropzone.options.lampAfterDropzone = {
    url: '{{ route('admin.rooms.storeMedia') }}',
    maxFilesize: 10, // MB
    acceptedFiles: '.jpeg,.jpg,.png,.gif',
    addRemoveLinks: true,
    ...
    success: function (file, response) {
      $('form').append('<input type="hidden" name="lamp_after[]" value="' + response.name + '">')
      uploadedLampAfterMap[file.name] = response.name
    },
    removedfile: function (file) {
      console.log(file)
      file.previewElement.remove()
      ...
      if (typeof file.file_name !== 'undefined') {
        ...
        name = uploadedLampAfterMap[file.name]
      }
      $('form').find('input[name="lamp_after[]"][value="' + name + '"]').remove()
    },
    init: function () {
@if(isset($room) && $room->lamp_after)
      var files = {!! json_encode($room->lamp_after) !!}
          for (var i in files) {
          var file = files[i]
          this.options.addedfile.call(this, file)
          this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
          file.previewElement.classList.add('dz-complete')
          $('form').append('<input type="hidden" name="lamp_after[]" value="' + file.file_name + '">')
        }
@endif
    },
     error: function (file, response) {
...
<script>
    var uploadedFanAfterMap = {}
Dropzone.options.fanAfterDropzone = {
    url: '{{ route('admin.rooms.storeMedia') }}',
    maxFilesize: 10, // MB
    acceptedFiles: '.jpeg,.jpg,.png,.gif',
    addRemoveLinks: true,
    ...
    success: function (file, response) {
      $('form').append('<input type="hidden" name="fan_after[]" value="' + response.name + '">')
      uploadedFanAfterMap[file.name] = response.name
    },
    removedfile: function (file) {
      console.log(file)
      file.previewElement.remove()
      ...
      if (typeof file.file_name !== 'undefined') {
        ...
        name = uploadedFanAfterMap[file.name]
      }
      $('form').find('input[name="fan_after[]"][value="' + name + '"]').remove()
    },
    init: function () {
@if(isset($room) && $room->fan_after)
      var files = {!! json_encode($room->fan_after) !!}
          for (var i in files) {
          var file = files[i]
          this.options.addedfile.call(this, file)
          this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
          file.previewElement.classList.add('dz-complete')
          $('form').append('<input type="hidden" name="fan_after[]" value="' + file.file_name + '">')
        }
@endif
    },
     error: function (file, response) {
