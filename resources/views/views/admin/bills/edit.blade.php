    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bills.update", [$bill->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ground') ? 'is-invalid' : '' }}" type="text" name="ground" id="ground" value="{{ old('ground', $bill->ground) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('line') ? 'is-invalid' : '' }}" type="text" name="line" id="line" value="{{ old('line', $bill->line) }}">
                @if($errors->has('line'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('power_detected') ? 'is-invalid' : '' }}" type="text" name="power_detected" id="power_detected" value="{{ old('power_detected', $bill->power_detected) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('panal') ? 'is-invalid' : '' }}" type="text" name="panal" id="panal" value="{{ old('panal', $bill->panal) }}">
                @if($errors->has('panal'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reading') ? 'is-invalid' : '' }}" type="text" name="reading" id="reading" value="{{ old('reading', $bill->reading) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('transformer') ? 'is-invalid' : '' }}" name="transformer_id" id="transformer_id">
                    @foreach($transformers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('transformer_id') ? old('transformer_id') : $bill->transformer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
