    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.cbs.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('transe') ? 'is-invalid' : '' }}" name="transe_id" id="transe_id">
                    @foreach($transes as $id => $entry)
                        <option value="{{ $id }}" {{ old('transe_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('trans_cb_fider_number') ? 'is-invalid' : '' }}" type="text" name="trans_cb_fider_number" id="trans_cb_fider_number" value="{{ old('trans_cb_fider_number', '') }}" required>
